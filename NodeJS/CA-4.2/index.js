...
app.get("/cars/:car", (req, res) => {
  ...
  // const newCar = car.charAt(0).toLocaleUpperCase() + car.slice(1);
  const userCars = data.filter(
    (user) => user.car.toLocaleUpperCase() === car.toLocaleUpperCase()
  );

  res.send(userCars).end();
});
...
app.get("/gender/:gender", (req, res) => {
  ...
  const filterGender = data.filter(
    (user) => user.gender.toLocaleLowerCase() === gender.toLocaleLowerCase()
  );

  const genderNames = filterGender.map(
    (user) => `${user.first_name} ${user.last_name}`
  );

  res.send(genderNames).end();
  return;
});
