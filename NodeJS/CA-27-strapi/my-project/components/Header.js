  ...
  return (
    <div className="border">
      <div className="flex fixed z-50 bg-white w-full border items-center justify-between py-5 px-5">
        <div>
          <Link href="/">
            <span className="text-3xl">Book Ratings</span>
          </Link>
        </div>
        <div className="flex items-center space-x-5">
          ...
          {isLoggedIn ? (
            <>
              <span
                onClick={logout}
                className="p-2 cursor-pointer border border-red-600"
              >
                Logout
              </span>
              <Link href="/create">
                <span className="text-red-500 underline">Create Book</span>
              </Link>
              <span className="p-2 w-10 h-10 block border rounded-full uppercase text-center">
