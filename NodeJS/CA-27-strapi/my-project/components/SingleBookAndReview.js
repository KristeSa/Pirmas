    ...
    <div className="mx-5">
      <div className="my-5" key={book.id}>
        <div className="flex items-center">
          ...
          <GetRatings value={reviews.averageScore} />({reviews.reviewsCount}{" "}
          reviews)
          {(book?.attributes?.creator === authUser?.username ||
            authUser?.isAdmin) && (
            <>
              <Link href={`/edit-book?book=${book.id}`}>
                <button
                  type="button"
                  className="ml-5 p-1 rounded bg-green-500 text-white w-14 shadow"
                >
                  Edit
                </button>
              </Link>
              <button type="button" onClick={handleDelete}>
                <button
                  type="button"
                  className="ml-5 p-1 rounded bg-red-500 text-white w-14 shadow"
                >
                  delete
                </button>
              </button>
        ...
        {reviews?.reviews?.length ? (
          reviews.reviews.map((review) => (
            <div key={review.id} className="my-3 border p-5">
              ...
              <p>
                {review?.comment ? (
                  <ReactQuill value={review.comment} readOnly theme="bubble" />
                ) : (
