      ...
      <div className="mx-5">
        {books?.length ? (
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {books.map((book) => (
              <div
                className="border relative rounded shadow my-5 p-5"
                key={book.id}
              >
                ...
                <div className="flex items-center bottom-0 left-0 absolute w-full p-2 justify-between">
                  ...
                  <div className="relative">
                    <div>
                      <span
                        id={book.id.toString() + book.id}
                        onClick={() => {
                          handleLikeBook(book, book.id);
                        }}
                        className={`${
                          book?.attributes?.likes?.includes(authUser?.username)
                            ? " liked "
                            : " "
                        } border heart-icon outline-none border-none block`}
                      />
                    </div>
                    <div className="absolute right-5 top-8">
                      <span
                        id={book.id.toString() + book.id + book.id}
                        className=""
                      >
                        {book?.attributes?.likes?.length || 0}
                      </span>
                    </div>
                  </div>
                </div>
