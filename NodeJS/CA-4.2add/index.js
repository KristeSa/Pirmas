...
app.get("/email", (req, res) => {
  ...
  res.send(email);
  // const userEmails = data.filter((user) => {
  //   user.email === email;
  // });

  // const allEmails = userEmails.map((user) => user.email);
  // res.send(allEmails).end();
  //return;
});
...
app.get("/gender/:gender", (req, res) => {
  ...
  const userFemale = filterGender.map(
    (user) => `${user.first_name} ${user.last_name}`
  );
  res.send(userFemale).end();
  return;
});
